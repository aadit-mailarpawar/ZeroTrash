...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const current = await getRawDb().prepare("SELECT status FROM reports WHERE id = ?").bind(id).first<{ status: string }>();
    ...
    const photo = requireImage((await request.formData()).get("photo"));
    key = imageKey("after", photo);
    const bucket = getBucket();
    await bucket.put(key, await photo.arrayBuffer(), { httpMetadata: { contentType: photo.type } });
    await getRawDb().prepare("UPDATE reports SET after_key = ?, status = 'awaiting_weighing' WHERE id = ?").bind(key, id).run();
