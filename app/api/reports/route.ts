...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await getRawDb().prepare("INSERT INTO reports (location, waste_type, notes, before_key, status) VALUES (?, ?, ?, ?, 'awaiting_cleanup') RETURNING id").bind(location, wasteType, notes, key).first<{ id: number }>();
