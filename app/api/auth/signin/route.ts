...
export async function POST(request: Request) {
  try {
    ...
    const user = await getRawDb().prepare("SELECT id, name, email, password_hash AS passwordHash, salt FROM users WHERE email = ?").bind(email).first<{ id: number; name: string; email: string; passwordHash: string; salt: string }>();
    ...
    return Response.json({ user: { name: user.name, email: user.email } }, { headers: { "Set-Cookie": cookie } });
  } catch (error) {
